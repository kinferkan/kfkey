...
function createNestedObject(keys, value) {
  ...
  const obj = {};
  obj[key] = createNestedObject(rest, value);
  return obj;
}
...
function main() {
  ...
  // 为英文文件生成缺失的翻译内容
  const enMissingContent = {};
  enMissingKeys.forEach(key => {
    const value = getValueByPath(zhData, key);
    const englishValue = translateToEnglish(value); // 简单的英文翻译占位符
    const keys = key.split('.');
    const nestedObj = createNestedObject(keys, englishValue);
    mergeNestedObjects(enMissingContent, nestedObj);
  });
  
  console.log(JSON.stringify(enMissingContent, null, 2));
  ...
  // 为德文文件生成缺失的翻译内容
  const deMissingContent = {};
  deMissingKeys.forEach(key => {
    const value = getValueByPath(zhData, key);
    const germanValue = translateToGerman(value); // 简单的德文翻译占位符
    const keys = key.split('.');
    const nestedObj = createNestedObject(keys, germanValue);
    mergeNestedObjects(deMissingContent, nestedObj);
  });
  
  console.log(JSON.stringify(deMissingContent, null, 2));
}

// 简单的英文翻译占位符函数
function translateToEnglish(chineseText) {
  // 这里应该使用真实的翻译服务，现在只是占位符
  const translations = {
